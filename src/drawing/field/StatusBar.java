...
public class StatusBar extends Field {
	...
	public void update() {
		...
		if ((xPos >= 529 && xPos <= 590 && yPos >= 30 + 550 && yPos <= 90 + 550 && InputUtility.isMouseClick())
			...
			// use MixPotion
			System.out.println("use MixPotion");
			inventory.use(Inventory.getInventory().get(2));
		} else if (xPos >= Constant.SCENE_WIDTH - 192 && xPos <= Constant.SCENE_WIDTH - 124 && yPos >= 25+550
				&& yPos <= 95 + 550 && InputUtility.isMouseClick() || (InputUtility.isKeyPressed(KeyCode.B))) {
		...
		} else if (xPos >= Constant.SCENE_WIDTH - 106 && xPos <= Constant.SCENE_WIDTH - 38 && yPos >= 25 + 550
				&& yPos <= 95 + 550 && InputUtility.isMouseClick() || (InputUtility.isKeyPressed(KeyCode.ENTER))) {
			InputUtility.mouseX = 0;
			InputUtility.mouseY = 0;
			BattleFieldMain.stop();
			// ResourceLoader.clickSound.play(100);
